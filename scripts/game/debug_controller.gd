...
func run_debug_play() -> void:
	...
	while _state.phase == PHASE_SETUP:
		if _state.setup_sub_phase == 0:  # PLACE_SETTLEMENT
			var slot = AIPlayer.pick_setup_vertex(_main.get_vertex_slots(), _state.tile_data, _state)
			if slot:
				_main.click_vertex_slot(slot)
			else:
				print("[DBGPLAY] ERROR: no valid setup vertex")
				break
		else:  # PLACE_ROAD
			var road_slot: Object = _main.find_setup_road_slot()
			if road_slot:
				_main.click_edge_slot(road_slot)
			else:
	...
	if DevCards.Type.KNIGHT not in p0.dev_cards:
		_state.debug_take_dev_card(0, DevCards.Type.KNIGHT)
	_state.current_player_index = 0
	_state.phase = PHASE_BUILD
	_main.try_play_dev_card_for(p0, DevCards.Type.KNIGHT, 0)
	...
	_state.debug_take_dev_card(0, DevCards.Type.YEAR_OF_PLENTY)
	_state.phase = PHASE_BUILD
	_main.try_play_dev_card_for(p0, DevCards.Type.YEAR_OF_PLENTY, 0)
	...
	var roads_placed := 0
	for _i in range(5):
		var slot = AIPlayer.pick_road(_main.get_edge_slots(), _main.get_vertex_slots(), p0, _state)
		if slot == null:
			print("[DBGPLAY] No more connected road slots available")
			break
		_ensure_road_resources(0)
		_main.click_edge_slot(slot)
		roads_placed += 1
...
func run_full_game() -> void:
	...
	while _state.phase != PHASE_GAME_OVER:
		...
		match _state.phase:
			PHASE_ROLL:
				...
				if _state.phase == PHASE_ROBBER_MOVE:
					...
					_main.set_tile_picking_enabled(false)
					_state.move_robber(robber_key)
					_main.refresh_robber_position()
			...
			PHASE_BUILD:
				...
				while _state.phase == PHASE_BUILD and actions_this_turn < 5:
					...
					var decision: Dictionary = AIPlayer.decide_build(
						player, _state, _main.get_vertex_slots(), _main.get_edge_slots())
					if decision.action == "end_turn":
						break
					print("[FULLGAME]   AI action: %s" % decision.action)
					match decision.action:
						"city":
							_main.click_vertex_slot(decision.params.slot)
						"settlement":
							_main.click_vertex_slot(decision.params.slot)
						"road":
							_main.click_edge_slot(decision.params.slot)
						"dev_card":
		...
		_main.stop_ai_turn_timer()  # kill any timer that fired during the await
...
func _auto_place_settlement(slot_idx: int) -> void:
	if slot_idx >= _main.get_vertex_slots().size():
		print("[DBGPLAY] WARN: vertex slot %d out of range" % slot_idx)
		return
	var p_name: String = _state.current_player().player_name

	# 1. Place settlement
	var slot = _main.get_vertex_slots()[slot_idx]
	if slot.is_occupied or not _state.passes_distance_rule(slot.position):
		# Find best free slot respecting distance rule
		slot = AIPlayer.pick_setup_vertex(_main.get_vertex_slots(), _state.tile_data, _state)
		if slot == null:
...
func _try_place_settlement_paid(player_idx: int, slot_idx: int) -> void:
	...
	if slot.is_occupied:
		# find nearest free
		for i in _main.get_vertex_slots().size():
			if not _main.get_vertex_slots()[i].is_occupied:
				slot = _main.get_vertex_slots()[i]
				break
	_main.click_vertex_slot(slot)
...
func _try_upgrade_city(player_idx: int, slot_idx: int) -> void:
	...
	if not (slot.is_occupied and slot.owner_index == player_idx and not slot.is_city):
		for s in _main.get_vertex_slots():
...
func _try_place_road(player_idx: int, _hint_idx: int = 0) -> void:
	...
	var placed := false
	for i in _main.get_edge_slots().size():
		var slot = _main.get_edge_slots()[i]
		if slot.is_occupied:
			continue
		_main.click_edge_slot(slot)
		if slot.is_occupied:
...
func _ensure_road_resources(player_idx: int) -> void:
	...
	if p.resources.get(PlayerData.RES_LUMBER, 0) < 1:
		_state.debug_adjust_resource(player_idx, PlayerData.RES_LUMBER, 1)
	if p.resources.get(PlayerData.RES_BRICK, 0) < 1:
		_state.debug_adjust_resource(player_idx, PlayerData.RES_BRICK, 1)
...
func _force_roll(number: int) -> void:
	...
	if number == 7:
		...
		_main.set_tile_picking_enabled(true)
	else:
		_state.debug_collect(number)
		_state.phase = PHASE_BUILD
	_main.refresh_hud_view()
...
func _give_resources(player_idx: int, amounts: Dictionary) -> void:
	var p = _state.players[player_idx]
	for r in amounts:
		_state.debug_adjust_resource(player_idx, int(r), int(amounts[r]))
